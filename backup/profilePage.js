...
const activateEditMode = (field) => {
    ...
    const linkElement = document.getElementById(`${field}Link`);

    if (field === "social") {
        if (linkElement) {
            inputElement.value = linkElement.textContent;
...
const revealPhoneNumber = (display) => {
    ...
    const lastThreeDigits = phoneNumberCopy.slice(-3); // Output: 234
...
const validateInput = (field) => {
    ...
    if (field === "social") {
        const linkPattern =
            /^(?:https:\/\/www\.)?(?:(?:instagram|x|facebook)\.com\/|linkedin\.com\/(?:in\/))[a-zA-Z0-9.]+\/?$/;
        const isValidLink = linkPattern.test(inputElement.value);
        return inputElement.value === "" ? true : isValidLink;
    }
};
...
const updateSaveButtonState = (field) => {
    ...
    const noChanges =
        ([...dataToBeUpdated.values()].every((value) => value === "") && isDefaultImage) || inputsInvalidLength === 0;
...
const acceptedSocialLink = {
    ...
    x: ["https://www.x.com/", "https://x.com/", "x.com/"],
};
...
for (const input of [
    ...
    input.addEventListener("blur", () => {
        ...
        // const platform =
        //     field === "social"
        //         ? input.value.includes("https://www")
        //             ? input.value.split(".")[1]
        //             : input.value.split(".")[0]
        //         : null;

        // const isLinkAccepted =
        //     field === "social"
        //         ? platform === "linkedin"
        //             ? input.value.includes("/in/")
        //             : acceptedSocialLink[platform].some((link) => input.value.includes(link))
        //         : null;

        // console.log(isLinkAccepted);

        // if (!isLinkAccepted) {
        //     showNotification("error", "Unaccepted social media link");
        // }
        ...
        if (field === "email" || field === "phone" || field === "social") {
            ...
            if (!validationResults[`isValid${fieldName}`]) {
                ...
                field !== "social"
                    ...
                    : showNotification("error", "Unaccepted link");
                updateSaveButtonState(field);
                return;
            }
        }
...
if (linkElement) {
    linkElement.href = linkElement.textContent.includes("https://")
        ? linkElement.textContent
        : `https://${linkElement.textContent}`;
}
